...
from huffman import avg_length, tree_to_bytes
from nodes import HuffmanNode
from huffman import num_nodes_to_bytes, compress, uncompress
...
class TestCompressionCode(unittest.TestCase):
    ...
    # since a complete tree has one fewer interior nodes than
    ...
    # object that is no longer than the input bytes
...
class TestRoundTrip(unittest.TestCase):
    ...
    # sample bytes: minimum, average, and maximum size
    @given(binary(1, 100, 1000))
    def test_round_trip_generate_compressed(self, b):
        text = b
        ...
        uncompressed = generate_uncompressed(tree, compressed, len(text))
